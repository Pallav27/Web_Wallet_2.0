...
export async function POST(req: Request, context: any) {
  try {
    ...
    // Start transaction with retry for transient write conflicts
    const maxRetries = 4;
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (isTransientError(err) && attempt < maxRetries) {
          console.warn(`Transient transaction error during fulfill, retrying (${attempt + 1}/${maxRetries})`, err?.message ?? err);
          await sleep(100 * Math.pow(2, attempt));
          continue;
        }
        console.error("Fulfill transaction failed:", err);
